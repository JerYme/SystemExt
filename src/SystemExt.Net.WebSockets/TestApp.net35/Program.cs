...
namespace TestApp
{
    class Program
    {
        ...
        const string WS_LOCAL_SERVER = "ws://127.0.0.1";
        ...
        static Task TestManagedClientWebSocketConnection(string server)
        {
            var ws = new ClientWebSocket();

            var buffer = new ArraySegment<byte>(new byte[1024*4]);
            const string msg = "hellohellohellohellohellohello";
