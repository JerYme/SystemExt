...
namespace System.IO
{
    ...
    [Serializable]
    [ComVisible(true)]
    public class StreamReaderPeekable : TextReader
    {
        // StreamReader.Null is threadsafe.
        public new static readonly StreamReaderPeekable Null = new NullStreamReader();
        ...
        internal static int DefaultBufferSize { get; } = 1024;
        ...
        internal StreamReaderPeekable()
        {
        }

        public StreamReaderPeekable(Stream stream)
            : this(stream, true)
        {
        }

        public StreamReaderPeekable(Stream stream, bool detectEncodingFromByteOrderMarks)
            : this(stream, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize, false)
        {
        }

        public StreamReaderPeekable(Stream stream, Encoding encoding)
            : this(stream, encoding, true, DefaultBufferSize, false)
        {
        }

        public StreamReaderPeekable(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks)
        ...
        // 
        public StreamReaderPeekable(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize)
            : this(stream, encoding, detectEncodingFromByteOrderMarks, bufferSize, false)
        {
        }

        public StreamReaderPeekable(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize, bool leaveOpen)
        {
        ...
        [Pure]
        public ArraySegment<char> PeekCharBuffer()
        {
            if (_stream == null)
                ReaderClosed();
            ...
            if (_charPos == _charLen)
            {
                return new ArraySegment<char>(_charBuffer, 0, 0);
            }
            return new ArraySegment<char>(_charBuffer, _charPos, _charLen - _charPos);
        }

        [Pure]
        public ArraySegment<byte> PeekByteBuffer()
        {
            if (_stream == null)
                ReaderClosed();
            ...
            if (_charPos == _charLen)
            {
                return new ArraySegment<byte>(_byteBuffer, 0, 0);
            }
            var offset = _encoding.GetByteCount(_charBuffer, _charPos, _charLen - _charPos);
            return new ArraySegment<byte>(_byteBuffer, offset, _byteLen - offset);
        }

        [Pure]
        public ArraySegment<byte> GetBytesFromCharBuffer()
        {
            var charBuffer = PeekCharBuffer();
            if (charBuffer.Count == 0) return new ArraySegment<byte>(_byteBuffer, 0, 0);
            var newBuffer = new byte[_encoding.GetByteCount(_charBuffer, _charPos, _charLen - _charPos)];
            var count = _encoding.GetEncoder().GetBytes(_charBuffer, _charPos, _charLen - _charPos, newBuffer, 0, true);
            return new ArraySegment<byte>(newBuffer, 0, count);
        }
        ...
        public override Task<string> ReadLineAsync()
        {
            ...
            if (GetType() != typeof(StreamReaderPeekable))
        ...
        private class NullStreamReader : StreamReaderPeekable
        {
