...
{
    internal sealed class ManagedWebSocketHandle
    {
        ...
        public static ManagedWebSocketHandle Create() => new ManagedWebSocketHandle();

        public static bool IsValid(ManagedWebSocketHandle handle) => handle != null;
        ...
        public Task ConnectAsyncCore(Uri uri, CancellationToken cancellationToken, ClientWebSocketOptions options)
        {
            ...
            CancellationTokenRegistration registration = cancellationToken.Register(s => ((ManagedWebSocketHandle)s).Abort(), this);
        ...
        private Task ConvertSocketToWebSocket(Uri uri, CancellationToken cancellationToken, ClientWebSocketOptions options, Socket connectedSocket)
        {
            return ConvertStreamToNetworkStream(uri, options, connectedSocket)
                .ContinueWithTask(t1 =>
                {
                    ...
                    var requestHeader = BuildRequestHeader(uri, options, secKeyAndSecWebSocketAccept.Key);
                ...
                .ContinueWith(tr =>
                {
                    ...
                    var r = tr.Result;
                    _webSocket = ManagedWebSocket.CreateFromConnectedStream(r.stream, false, r.subprotocol, options.KeepAliveInterval, options.ReceiveBufferSize, options.Buffer);
        ...
        private static Task<Stream> ConvertStreamToNetworkStream(Uri uri, ClientWebSocketOptions options, Socket connectedSocket)
        {
            Stream stream = new NetworkStream(connectedSocket, true);
            if (uri.Scheme != UriScheme.Wss) return TaskEx.FromResult(stream);

            // Upgrade to SSL if needed
            var sslStream = new SslStream(stream);
            return sslStream.AuthenticateAsClientAsync(uri.Host, options.ClientCertificates, SecurityProtocol.AllowedSecurityProtocols, false)
                   .ContinueWith(t1 => (Stream)sslStream, TaskContinuationOptions.ExecuteSynchronously);
        }
                    ...
                    var tasks = Array.ConvertAll(addresses, address =>
                    {
                        ...
                        return socket.ConnectAsync(address, port)
                            .CatchWith((ObjectDisposedException ode) =>
                            {
                                ...
                                {
                                    throw new OperationCanceledException(new OperationCanceledException().Message, ode);
                                }
                            }).ContinueWith(t =>
                            {
        ...
        private Task<string> ParseAndValidateConnectResponseAsync(
        ...
        private static Task<string> ReadResponseHeaderLineAsync(Stream stream, CancellationToken cancellationToken)
        {
            ...
            return TaskEx.AsyncLoopTask(() => stream.ReadAsync(arr, 0, 1, cancellationToken)
                    .ContinueWith(task =>
                    {
                        var i = task.Result;
